...
static void xboard_step    (char string[]);
static void engine_step    (char string[]);
...
#ifdef _WIN32
static void adapter_step(){  // polling!
    bool xin,ein;
    char string[StringSize];
    if(option_get_bool("UCI")){
        xin=gui_get_non_blocking(GUI,string,StringSize);
        if(xin) uci_gui_step(string);
        ein=engine_get_non_blocking(Engine,string,StringSize);
        if(ein) uci_engine_step(string);
    }else{
        xin=gui_get_non_blocking(GUI,string,StringSize);
        if(xin) xboard_step(string);
        ein=engine_get_non_blocking(Engine,string,StringSize);
        if(ein) engine_step(string);
    }
...
static void adapter_step() {
   ...
   while (io_line_ready(GUI->io)){
      ...
      if(option_get_bool("UCI")){
           ...
           xboard_step(string); // process available xboard lines
       }
   }
   while (io_line_ready(Engine->io)){
       engine_get(Engine,string,StringSize);
       if(option_get_bool("UCI")){
           uci_engine_step(string); // process available engine lines
       }else{
           engine_step(string);
       }
   }
