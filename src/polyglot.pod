...
repetition, etc ... It also supports Chess960 and some variants.
...
By definition a line is a sequence of moves (from the starting position)
in which the given color makes only book moves and the other color makes
arbitrary moves (i.e. not necessarily book moves).

Since a PolyGlot book is comprised of positions (and not of "lines") there
may be positions in the book that are not on a "line" as previously defined.
It is convenient to call such positions "isolated" positions.
The utility "info-book" counts such isolated positions.

Some isolated positions are provably unreachable and they could in principle
be deleted from the book. For example if a book contains only the move "e4"
in the starting position but also the position after "d4 d5" then this last
position is provably unreachable from "lines" since it requires white to make
a non-book move when a book move is available. Such situations arise from
improper pruning during merging opening books -- which will be corrected.

If invoked with "-exact" the -utility info-book will count isolated keys
which are discovered to be unreachable from "lines". Due to the possibility
of transpositions (after finishing "lines") this is not a foolproof method.
...
TODO: Where positions are identical, preserve moves and scale "in2" weights
...
=item B<SyzygyPath> (default: <empty>)

Directory or directories where the .rtbw and .rtbz files can be found.
Multiple directories should be separated by ";" on Windows and by ":"
on Unix-based operating systems.

=item B<SyzygyProbeDepth> (default: 1)

Determines the depth at which tablebases will be probed during search.

=item B<Syzygy50MoveRule> (default: true)

Determines whether to respect the 50-move rule during tablebase probing.

=item B<SyzygyProbeLimit> (default: 6)

Determines how few pieces need to be on the board before probing occurs.
...
Native Windows port:  Huang Chen<webmaster(at)elephantbase.net> ("Morning Yellow")
